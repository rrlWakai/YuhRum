...
export function BookingModal({ villaId, onClose }: Props) {
  ...
  function canProceed() {
    ...
    if (step === 2) return form.name.trim() && form.contact.trim() && form.email.trim();
  ...
  async function handleNext() {
    ...
    if (step === 1) {
      ...
      if (blockedDates.has(form.date)) {
        setErrorMsg('Sorry, this date was just booked by someone else. Please select another date.');
  ...
  const SummaryContent = () => (
    <div className="flex flex-col h-full">
      <h3 className="font-serif text-2xl text-[#0A192F] mb-6 hidden md:block">Booking Summary</h3>
      
      <div className="flex-1 space-y-6">
        <div className="flex items-start gap-4">
          <img src={selectedVilla.coverImage} alt={selectedVilla.name} className="w-24 h-24 object-cover rounded-lg border border-gray-200" />
          <div>
            <p className="text-[10px] uppercase tracking-widest text-gray-500">Private Villa</p>
            <p className="font-serif text-lg text-[#0A192F]">{selectedVilla.name}</p>
            <p className="text-sm text-gray-600 mt-1">{STAY_LABELS[form.stayType].label}</p>
          </div>
        </div>
        ...
        <div className="border-t border-gray-200 pt-6 space-y-4 text-sm text-gray-700">
          <div className="flex justify-between">
            <span>Date</span>
            <span className="font-medium text-[#0A192F]">{form.date || 'Not selected'}</span>
          </div>
          <div className="flex justify-between">
            <span>Guests</span>
            <span className="font-medium text-[#0A192F]">{form.guests} pax</span>
          </div>
        </div>
      ...
      <div className="mt-6 border-t border-[#D1DEEA] pt-6">
        <div className="flex justify-between items-end mb-1">
          <span className="text-xs font-semibold uppercase tracking-[0.15em] text-[#0A192F]">Total Due</span>
          <span className="font-serif text-3xl text-[#0A192F]">{formatPrice(total)}</span>
        </div>
        ...
        >
          {isSuccess ? (
            <div className="flex-1 flex flex-col items-center justify-center p-10 text-center bg-white">
              ...
              <h2 className="font-serif text-4xl text-[#0A192F]">Booking Request Sent</h2>
              <p className="mt-4 max-w-sm text-gray-600 leading-relaxed">
                Your reservation request is confirmed. We've sent a detailed email with your itinerary and payment instructions.
              </p>
              <button onClick={onClose} className="mt-10 btn-navy px-10 py-4 text-xs font-semibold uppercase tracking-[0.15em]">
                Return to Website
              </button>
            </div>
          ) : (
              ...
              <div className="flex-1 flex flex-col h-full bg-white relative overflow-hidden">
                ...
                <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100 bg-white z-10 md:px-10 md:py-6 md:border-none">
                  <div>
                    ...
                    <p className="font-serif text-2xl text-[#0A192F] mt-1">{STEP_LABELS[step - 1]}</p>
                  </div>
                  <button onClick={onClose} className="flex size-10 items-center justify-center rounded-full bg-gray-50 text-gray-500 hover:bg-gray-100 transition-colors">
                    <X className="size-5" />
                  </button>
                </div>
                ...
                <div className="flex bg-gray-100 h-1 w-full">
                  {STEP_LABELS.map((_, i) => (
                    <div key={i} className={`h-full flex-1 transition-colors duration-500 ${i + 1 <= step ? 'bg-[#0A192F]' : 'bg-transparent'}`} />
                  ))}
                </div>
                  ...
                  <AnimatePresence mode="wait">
                    {step === 1 && (
                      <motion.div key="step1" initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -10 }} className="space-y-8">
                        ...
                        <div>
                          ...
                          <div className="grid grid-cols-3 gap-3">
                            {(Object.keys(STAY_LABELS) as StayType[]).map((type) => {
                              ...
                              return (
                                <button
                                  key={type} type="button" onClick={() => update('stayType', type)}
                                  className={`flex flex-col items-center justify-center p-4 rounded-xl border-2 transition-all duration-200 ${
                                    active ? 'border-[#0A192F] bg-[#0A192F] text-white shadow-md' : 'border-gray-100 bg-white text-gray-500 hover:border-gray-200'
                                  }`}
                                >
                                  <Icon className={`size-5 mb-2 ${active ? 'text-white' : 'text-[#0A192F]'}`} />
                                  <span className="text-xs font-bold uppercase tracking-wider">{label}</span>
                                  <span className={`text-[9px] uppercase tracking-widest mt-1 ${active ? 'text-white/70' : 'text-gray-400'}`}>{sub}</span>
                                </button>
                              );
                            })}
                          </div>
                        ...
                        <div>
                          ...
                          {form.date && (
                            <motion.p initial={{ opacity: 0, y: -5 }} animate={{ opacity: 1, y: 0 }} className="mt-3 text-[11px] font-medium uppercase tracking-widest text-[#3D6B52] flex items-center gap-1.5">
                              <Check className="size-3" /> Date selected. {isWknd ? 'Weekend' : 'Weekday'} rate applies.
                            </motion.p>
                          )}
                        </div>
                        ...
                        <div>
                          ...
                          <div className="relative">
                            ...
                            <input
                              type="number" min={1} max={rate.capacity} value={form.guests}
                          ...
                          <p className="mt-2 text-[10px] uppercase tracking-widest text-gray-500">Max {rate.capacity} guests. +₱500 per extra guest after {selectedVilla.capacity.base}.</p>
                        </div>
                      </motion.div>
                    )}
                    ...
                    {step === 2 && (
                      <motion.div key="step2" initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -10 }} className="space-y-6">
                        <div>
                          <label className="mb-2 block text-xs font-semibold uppercase tracking-[0.15em] text-[#0A192F]">Full Name</label>
                          <div className="relative">
                            <div className="absolute inset-y-0 left-0 flex items-center pl-5 pointer-events-none text-[#0A192F]/40"><User className="size-5" /></div>
                            <input
                              value={form.name} onChange={(e) => update('name', e.target.value)} placeholder="Juan dela Cruz"
                              className="w-full rounded-xl border-2 border-gray-100 bg-white pl-14 pr-5 py-4 text-sm font-medium text-[#0A192F] outline-none transition-all focus:border-[#0A192F] focus:ring-4 focus:ring-[#0A192F]/10"
                            />
                          </div>
                        </div>
                        <div>
                          <label className="mb-2 block text-xs font-semibold uppercase tracking-[0.15em] text-[#0A192F]">Contact Number</label>
                          <div className="relative">
                            <div className="absolute inset-y-0 left-0 flex items-center pl-5 pointer-events-none text-[#0A192F]/40"><Phone className="size-5" /></div>
                            <input
                              value={form.contact} onChange={(e) => update('contact', e.target.value)} placeholder="+63 912 345 6789"
                              className="w-full rounded-xl border-2 border-gray-100 bg-white pl-14 pr-5 py-4 text-sm font-medium text-[#0A192F] outline-none transition-all focus:border-[#0A192F] focus:ring-4 focus:ring-[#0A192F]/10"
                            />
                          </div>
                        </div>
                        <div>
                          <label className="mb-2 block text-xs font-semibold uppercase tracking-[0.15em] text-[#0A192F]">Email Address</label>
                          <div className="relative">
                            <div className="absolute inset-y-0 left-0 flex items-center pl-5 pointer-events-none text-[#0A192F]/40"><Mail className="size-5" /></div>
                            <input
                              type="email" value={form.email} onChange={(e) => update('email', e.target.value)} placeholder="juan@email.com"
                              className="w-full rounded-xl border-2 border-gray-100 bg-white pl-14 pr-5 py-4 text-sm font-medium text-[#0A192F] outline-none transition-all focus:border-[#0A192F] focus:ring-4 focus:ring-[#0A192F]/10"
                            />
                          </div>
                        </div>
                        <div>
                          <label className="mb-2 block text-xs font-semibold uppercase tracking-[0.15em] text-[#0A192F]">Special Event? (Optional)</label>
                          <div className="relative">
                            <div className="absolute inset-y-0 left-0 flex items-center pl-5 pointer-events-none text-[#0A192F]/40"><CalendarHeart className="size-5" /></div>
                            <input
                              value={form.eventType} onChange={(e) => update('eventType', e.target.value)} placeholder="e.g. Birthday, Team Building..."
                              className="w-full rounded-xl border-2 border-gray-100 bg-white pl-14 pr-5 py-4 text-sm font-medium text-[#0A192F] outline-none transition-all focus:border-[#0A192F] focus:ring-4 focus:ring-[#0A192F]/10"
                            />
                          </div>
                        </div>
                    ...
                    {step === 3 && (
                      <motion.div key="step3" initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -10 }} className="space-y-8">
                        ...
                        <div>
                          <label className="mb-3 block text-xs font-semibold uppercase tracking-[0.15em] text-[#0A192F]">Payment Method</label>
                          <div className="grid grid-cols-2 gap-4">
                            {(['gcash', 'card'] as const).map((method) => (
                              <button
                                key={method} type="button" onClick={() => update('payMethod', method)}
                                className={`flex flex-col items-center justify-center p-4 rounded-xl border-2 transition-all duration-200 ${
                                  form.payMethod === method ? 'border-[#0A192F] bg-[#0A192F] text-white shadow-md' : 'border-gray-100 bg-white text-gray-500 hover:border-gray-200'
                                }`}
                              >
                                {method === 'gcash' ? <Smartphone className={`size-6 mb-2 ${form.payMethod === method ? 'text-white' : 'text-[#0A192F]'}`} /> : <CreditCard className={`size-6 mb-2 ${form.payMethod === method ? 'text-white' : 'text-[#0A192F]'}`} />}
                                <span className="text-xs font-bold uppercase tracking-wider">{method === 'gcash' ? 'GCash' : 'Visa / Card'}</span>
                              </button>
                            ))}
                          </div>
                        </div>
                        ...
                        <label className="flex cursor-pointer items-start gap-4 p-4 rounded-xl border border-gray-100 bg-gray-50">
                          ...
                          <span className="text-xs leading-relaxed text-gray-600">
                            I agree to the booking terms and cancellation policy. I understand this is a reservation request and requires confirmation before the payment is fully settled.
                          </span>
                        </label>

                        <div className="flex items-center justify-center gap-2 text-[10px] uppercase tracking-widest text-gray-400">
                          <Shield className="size-4" /> Secure payment processing powered by <strong className="text-[#0A192F] font-bold">PayMongo</strong>
                        </div>
                      </motion.div>
                    )}
                  </AnimatePresence>
                ...
                <div className="absolute bottom-0 left-0 right-0 bg-white border-t border-gray-100 p-5 md:static md:p-6 md:border-none flex items-center justify-between z-20">
                  {step > 1 ? (
                    <button onClick={() => setStep((s) => s - 1)} className="flex items-center gap-2 rounded-xl bg-gray-50 px-6 py-3.5 text-xs font-bold uppercase tracking-wider text-[#0A192F] hover:bg-gray-100 transition-colors">
                  ...
                  {step < 3 ? (
                    <button 
                      onClick={handleNext} disabled={!canProceed() || isSubmitting} 
                    ...
                    >
                      {isSubmitting ? <Loader2 className="size-4 animate-spin" /> : 'Continue'} <ChevronRight className="size-4" />
                    </button>
                  ) : (
                    <button 
                      onClick={handleCompleteBooking} disabled={!canProceed() || isSubmitting} 
                      className="flex items-center gap-2 rounded-xl bg-[#0A192F] px-8 py-3.5 text-xs font-bold uppercase tracking-wider text-white shadow-lg shadow-[#0A192F]/20 hover:bg-[#0A192F]/90 transition-all disabled:opacity-50 disabled:shadow-none"
                    >
                      {isSubmitting ? <Loader2 className="size-4 animate-spin" /> : 'Confirm & Pay'}
                    </button>
                  )}
                </div>
                ...
                {errorMsg && (
                  <div className="absolute top-20 left-4 right-4 z-30">
                    <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="rounded-xl border border-red-200 bg-red-50 p-4 text-xs font-medium text-red-600 shadow-md">
                      {errorMsg}
                    </motion.div>
                  </div>
                )}
