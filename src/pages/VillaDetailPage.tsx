  ...
  return (
    <div className="bg-[#F7F6F4]">
      <section className="relative h-[65vh] min-h-480px overflow-hidden md:h-[75vh]">
        <motion.div
          style={{
            backgroundImage: `url(${allImages[activeThumb]})`,
            y: heroY,
          }}
        ...
        <div className="absolute bottom-0 left-0 right-0 z-10 px-6 pb-10 md:px-10">
          <div className="mx-auto max-w-1240px">
            ...
            >
              ...
              <h1 className="mt-4 font-serif text-4xl leading-tight text-white sm:text-5xl md:text-6xl lg:text-7xl">
                {villa.name}
              </h1>
              <div className="mt-3 flex flex-wrap items-center gap-5 text-sm text-white/85">
                ...
                <span className="flex items-center gap-2">
                  <Users className="size-4" /> Up to {villa.capacity.max} guests
                </span>
              </div>
            </motion.div>
            ...
            <div className="mt-8 flex items-center gap-2">
              {allImages.length > VISIBLE_THUMBS && (
                <button
                  ...
                  className="flex size-10 shrink-0 items-center justify-center bg-black/40 text-white disabled:opacity-30 hover:bg-black/60 transition-colors backdrop-blur-sm"
                >
              ...
              {allImages.length > VISIBLE_THUMBS && (
                <button
                  onClick={nextThumb}
                  disabled={thumbStart >= allImages.length - VISIBLE_THUMBS}
                  className="flex size-10 shrink-0 items-center justify-center bg-black/40 text-white disabled:opacity-30 hover:bg-black/60 transition-colors backdrop-blur-sm"
                >
        ...
        <div className="mx-auto flex max-w-1240px items-center justify-between px-6 py-4 md:px-10">
          <div className="hidden flex-wrap gap-2 md:flex">
            {villa.highlights.map((h) => (
              <span
                key={h}
                className="border border-gray-200 bg-[#F7F6F4] px-4 py-1.5 text-[10px] uppercase tracking-[0.15em] text-gray-600"
              >
                {h}
              </span>
            ))}
          ...
          <Reveal delay={0.05}>
            ...
            <div className="mt-8 border-l border-gray-300 pl-6">
              ...
              <p className="mt-3 text-sm text-gray-700">
                The entire villa is exclusively yours — no shared spaces, no
                other guests.
              </p>
            </div>
          </Reveal>
        ...
        <div className="mx-auto max-w-1240px">
          ...
          >
            {villa.amenities[amenityTab].map((item) => (
              <div
                key={item}
                className="flex items-center gap-4 border border-gray-200 bg-[#F7F6F4] p-5"
              >
        ...
        <div className="mx-auto max-w-1240px">
          <Reveal delay={0.05}>
            ...
            <p className="mt-4 text-gray-600">
              All rates are for the entire villa. +₱500 per extra guest beyond
              10 pax.
            </p>
          </Reveal>
        ...
        <div className="mx-auto max-w-1240px">
          <Reveal delay={0.05}>
            <div className="flex items-end justify-between">
              ...
              <button
                onClick={() =>
                  onNavigate({ type: "gallery", villaId: villa.id })
                }
      ...
      <Section
        id="availability"
        className="bg-[#F7F6F4] border-b border-gray-200"
      >
        <div className="mx-auto max-w-1240px">
          <Reveal delay={0.05}>
            <p className="text-xs uppercase tracking-[0.22em] text-gray-600">
            ...
            <p className="mt-4 text-gray-600">
              Select your preferred dates to check if the villa is available.
            </p>
          </Reveal>
              ...
              <div className="border border-gray-200 bg-white p-7">
                ...
                <div className="mt-6 space-y-4 text-sm">
                  <div className="flex justify-between border-b border-gray-100 pb-3">
                    ...
                    <span className="font-serif text-lg text-neutral-900">
                      {fmt(villa.rates.dayStay.weekday)}
                    </span>
                  </div>
                  <div className="flex justify-between border-b border-gray-100 pb-3">
                    <span className="text-gray-600">Day Stay (Weekend)</span>
                    <span className="font-serif text-lg text-neutral-900">
                      {fmt(villa.rates.dayStay.weekend)}
                    </span>
                  </div>
                  <div className="flex justify-between border-b border-gray-100 pb-3">
                    <span className="text-gray-600">Overnight (Weekday)</span>
                    <span className="font-serif text-lg text-neutral-900">
                      {fmt(villa.rates.overnight.weekday)}
                    </span>
                  </div>
                  <div className="flex justify-between pb-1">
                    <span className="text-gray-600">Overnight (Weekend)</span>
                    <span className="font-serif text-lg text-neutral-900">
                      {fmt(villa.rates.overnight.weekend)}
                    </span>
                  </div>
                  <p className="pt-2 text-xs text-gray-400 uppercase tracking-wide">
                    +₱500 per extra guest beyond 10 pax
                  </p>
                </div>
              </div>
              <button
            ...
            <Reveal delay={0.1} className="space-y-6">
              <div className="border border-gray-200 bg-[#F7F6F4] p-7">
                ...
                <div className="mt-6 space-y-3">
                  <a
                    href={`tel:${villa.contact.phone}`}
                    className="flex items-center gap-4 border border-gray-200 bg-white p-4 text-sm text-neutral-700 transition-colors hover:border-neutral-900 hover:text-neutral-900"
                  >
                  ...
                  <a
                    href={`https://${villa.contact.facebook}`}
                    target="_blank"
                  ...
                  <a
                    href={`https://instagram.com/${villa.contact.instagram.replace("@", "")}`}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex items-center gap-4 border border-gray-200 bg-white p-4 text-sm text-neutral-700 transition-colors hover:border-neutral-900 hover:text-neutral-900"
                  >
      ...
      <div className="sticky bottom-0 z-30 border-t border-gray-100 bg-white/95 px-5 py-4 backdrop-blur-xl md:hidden shadow-[0_-10px_30px_rgba(0,0,0,0.05)]">
        <div className="flex items-center justify-between gap-4">
          <div className="flex flex-col">
            ...
            <span className="font-serif text-xl text-[#0A192F] leading-none mt-1">
              {fmt(villa.rates.dayStay.weekday)}
            </span>
          </div>
          <button
